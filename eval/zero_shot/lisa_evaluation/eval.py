...
def evaluate_model(tasks):
    ...
    for task in tasks:
        ...
        ious = []
        screenspot_data = json.load(open(f"/path/to/your/lisa_{task}.json", 'r'))
        ...
        for item in tqdm(screenspot_data):
            ...
            try:
                ...
                ious.append(0)
        json.dump(box_res, open(f"tmp_ours_test/resbox_{os.environ['SPLIT']}_r1_w_think_7b.json", 'w'), indent=4)
        json.dump(ious, open(f"tmp_ours_test/res_{os.environ['SPLIT']}.json", 'w'), indent=4)
    return
